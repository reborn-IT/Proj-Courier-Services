...
function FAQComponent({ id, Question, Answer } :FAQDataInterface) {
  ...
  return (
    <>
      ...
      >
        <h4 className="text-md sm:text-xl font-semibold text-drop-grey">{Question}</h4>
        <img
          src={DownArrowIcon}
          alt="Down Arrow Icon"
          className={`transition-all duration-300 ease-in-out ${revealed ? 'rotate-180' : 'rotate-0'} w-7`}
        />
      </button>
      <div
        className={`
