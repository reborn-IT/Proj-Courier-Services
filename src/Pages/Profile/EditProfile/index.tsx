...
function EditProfile() {
  ...
  const [anotherAddressLineOne, setAnotherAddressLineOne] = useState<string>('');
  ...
  function addNewItem() {
    ...
    const currentArray = [...anotherAddresses, {
      id: addressID, title: addressTitle, lineOne: anotherAddressLineOne, lineTwo: anotherAddressLineTwo, city: anotherAddressCity,
    }];
  ...
  return (
    <div className="profile flex flex-col justify-center items-center mt-12 md:mt-20 2xl:mt-36">
      ...
      <div className="flex justify-center items-center w-full">
        <label htmlFor="dropzone-file" className="flex flex-col justify-center items-center w-full h-64 bg-gray-50 rounded-lg border-2 border-drop-primary border-dashed cursor-pointer hover:bg-gray-100">
      ...
      <form className="w-full mt-6">
        <div className="flex items-center justify-between w-full">
          ...
          <button onClick={() => setNewAddress(!newAddress)} type="button" className={`hover:scale-110 active:scale-90 transition-all duration-300 transform ${newAddress ? 'rotate-[135deg]' : 'rotate-0'}`}>
        ...
        <div className={`flex mt-7 p-5 flex-col ${anotherAddresses.length >= 1 ? 'justify-start items-start min-h-64' : 'justify-center items-center h-64'} w-full bg-gray-50 rounded-lg border-2 border-drop-primary border-dashed`}>
          {
            anotherAddresses.length < 1 ? (
              <>
                <p className="italic text-sm font-semibold text-drop-grey">Add Another address, save your time when filtering.</p>
                <p className="italic text-drop-textshaded text-xs">(Added addresses will appear here.)</p>
              </>
            ) : anotherAddresses.map(({
              id, title, lineOne, lineTwo, city,
            }) => (
              <div key={id} className="w-full text-drop-grey mb-2 flex shadow items-center bg-drop-white px-3 py-3 rounded-md">
