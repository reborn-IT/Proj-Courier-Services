...
const experienceContentVariants = {
  show: {
    opacity: 1,
    x: 0,
    position: 'relative',
...
const reviewWriteContentVariants = {
  show: {
    opacity: 1,
    x: 0,
    display: 'block',
...
function ReviewForm() {
  ...
  const [visibleHighlights, setVisibleHighlights] = useState<highlightI[]>([]);
  const [selectedHighlights, setSelectedHighlights] = useState<Set<unknown>>(new Set());
  const [selectedExperienceType, setSelectedExperienceType] = useState<ExperienceTypes>('angry');
  ...
  function handleMotionHighlights() {
    if (showHighlights) {
      ...
      setTimeout(() => { setShowHighlights(true); }, 150);
    }
  }
  ...
  return (
    <div className={`review-popup w-full h-full fixed z-50 ${reviewPopupState ? 'flex' : 'hidden'} items-center justify-center top-0 left-0 right-0 bottom-0 bg-drop-grey/20 backdrop-blur-md`}>
      <motion.div animate={reviewPopupState ? 'show' : 'hidden'} variants={reviewPopupVariants} className="relative rounded-xl bg-drop-white w-[30rem] py-5 px-6 overflow-hidden flex flex-col">
        <div className={`items-center justify-between ${currentStep === 'STEP_THREE' ? 'hidden' : 'flex'}`}>
          <div className="toggle-switch items-center space-x-3 flex">
            <p className="text-sm font-semibold text-drop-grey ">Post Anonymously</p>
            <Switch
              checked={isAnonymous}
              onColor="#525298"
              checkedIcon={false}
          ...
          <ModalCloseButton
            ClickHandler={() => closeReviewCard()}
          />
        </div>
        {/* Content */}
        <div className="w-full flex flex-col items-center mt-4">
          <img className="w-20 h-20 object-cover rounded-full" src={isAnonymous ? AnonymousPlaceholderImage : UserReviewImage} alt="" />
          <p className="mt-2 text-2xl font-semibold text-drop-primary">{isAnonymous ? 'Anonymous User' : 'Floyd Miles'}</p>
          <motion.div animate={currentStep === 'STEP_ONE' ? 'show' : 'hidden'} variants={experienceContentVariants} className="experience-section w-full mt-6">
            <p className="text-center text-drop-grey mb-4 font-semibold">How was your experience?</p>
          ...
          <motion.div animate={showHighlights ? 'show' : 'hidden'} variants={highlightVariants} className="experience-section w-full mt-6">
            <p className="text-center text-drop-grey mb-4 font-semibold">Highlights</p>

            <motion.div className="highlights-section flex flex-wrap">
              {
                    visibleHighlights && visibleHighlights.map(({ id, highlight }) => (
                      <Highlight
                        key={id}
            ...
            <motion.div animate={selectedHighlights && selectedHighlights.size > 0 ? 'show' : 'hidden'} variants={nextButtonVariants} className="w-full mt-6 mb-1 flex items-center justify-center">
              <CommonRoundedButton
                motionDiv
                ClickHandler={() => {
                  setCurrentStep('STEP_TWO'); setShowHighlights(false);
                }}
              >
        ...
        <motion.div animate={currentStep === 'STEP_TWO' ? 'show' : 'hidden'} variants={reviewWriteContentVariants} className="w-full flex flex-col items-center mt-4">
          <p className="text-center text-drop-grey mb-4 font-semibold">Write your review?</p>
          <form className="w-full">
            <textarea value={review} onChange={(e) => handleReviewInput(e.target.value)} cols={30} rows={10} placeholder="Write review..." className={`rounded-lg bg-drop-textshaded/10 py-4 px-6 w-full ${isTextareaValid ? '' : 'ring-drop-red ring-2 placeholder:text-drop-red'}`} />
          </form>
          <div className="flex items-center justify-center space-x-5 mt-5">
            <CommonRoundedButton extraTailwindClasses="bg-drop-red">
              No Thanks
            </CommonRoundedButton>
            <CommonRoundedButton ClickHandler={() => setCurrentStep('STEP_THREE')}>
              Post
            </CommonRoundedButton>
          </div>
        </motion.div>
        ...
        {/* Step THREE: Stars */}
        <motion.div animate={currentStep === 'STEP_THREE' ? 'show' : 'hidden'} variants={starsPopupVariants} className="w-full flex flex-col items-center mt-4">
          <p className="text-center text-xl text-drop-grey mb-4 font-semibold">Thank You for your feedback</p>
          <motion.div
          ...
          >
            <motion.i animate={currentStep === 'STEP_THREE' ? 'show' : 'hidden'} variants={singleStarVariants} className="bi bi-star-fill text-5xl text-drop-gold" />
            <motion.i animate={currentStep === 'STEP_THREE' ? 'show' : 'hidden'} variants={singleStarVariants} className="bi bi-star-fill text-5xl text-drop-gold" />
            <motion.i animate={currentStep === 'STEP_THREE' ? 'show' : 'hidden'} variants={singleStarVariants} className="bi bi-star-fill text-5xl text-drop-gold" />
            <motion.i animate={currentStep === 'STEP_THREE' ? 'show' : 'hidden'} variants={singleStarVariants} className="bi bi-star-fill text-5xl text-drop-gold" />
            <motion.i animate={currentStep === 'STEP_THREE' ? 'show' : 'hidden'} variants={singleStarVariants} className="bi bi-star-fill text-5xl text-drop-gold" />
